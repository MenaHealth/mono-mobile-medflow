// components/patientViewModels/PatientNotes/CombinedNotesViewModel.ts
...
export function CombinedNotesViewModel(patientId: string) {
    ...
    const [templateType, setTemplateType] = useState<'physician' | 'procedure' | 'subjective' | 'blank'>('physician');
    ...
    const [blankNote, setBlankNote] = useState<BlankNote>({
        date: new Date().toISOString().split('T')[0],
        time: new Date().toTimeString().split(' ')[0],
        noteField: ''
    });
    ...
    const setNoteField = useCallback((noteType: 'physician' | 'procedure' | 'subjective' | 'blank', name: string, value: string) => {
        switch (noteType) {
            case 'physician':
            ...
            case 'subjective':
                ...
                break;
            case 'blank':
                setBlankNote(prev => ({ ...prev, [name]: value }));
                break;
            default:
    ...
    const populateNote = useCallback((note: INote) => {
        ...
        switch (noteType) {
            ...
            case 'subjective':
                ...
                break;
            case 'blank':
                setNoteField('blank', 'noteField', note.content?.noteField);
                break;
            default:
                console.error('Invalid note type');
        }
    ...
    }, [templateType, physicianNote, procedureNote, subjectiveNote, blankNote, patientId, authorID, authorName, session, refreshPatientNotes]);
