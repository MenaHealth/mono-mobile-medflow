...
import {Pencil, X, Copy, Check, Info, Trash2, Plus} from 'lucide-react';
...
export function UserProfileView({ isAdmin = false, userId }: UserProfileViewProps) {
    ...
    const [specialties, setSpecialties] = useState<DoctorSpecialtyList[]>([]);
    useEffect(() => {
        if (vm.profile?.doctorSpecialty) {
            setSpecialties(vm.profile.doctorSpecialty);
        }
    }, [vm.profile?.doctorSpecialty]);
    ...
    const handleAddSpecialty = () => {
        if (specialties.length < 2) {
            setSpecialties([...specialties, DoctorSpecialtyList.ALLERGY_IMMUNOLOGY]); // Use a valid enum value
        }
    };

    const handleRemoveSpecialty = (index: number) => {
        setSpecialties(specialties.filter((_, i) => i !== index)); // Remove specialty at index
    };
                                ...
                                <div className="mt-4">
                                    ...
                                    {vm.isEditing
                                        ? specialties.map((specialty, index) => (
                                            <div key={index} className="flex mt-2">
                                                <div className="flex items-center space-x-2">
                                                    <SingleChoiceFormField
                                                        fieldName={`doctorSpecialty[${index}]`}
                                                        fieldLabel={`Specialty ${index + 1}`}
                                                        choices={DoctorSpecialties}
                                                        value={specialty}
                                                        onChange={(value: string) => vm.handleSpecialtyChange(index, value as DoctorSpecialtyList)}
                                                    ...
                                                    {index === 1 && (
                                                        <Button
                                                            ...
                                                            variant="ghost"
                                                            size="icon"
                                                            onClick={() => vm.handleRemoveSpecialty(specialty)} // Updated to call ViewModel
                                        ...
                                        : specialties.map((specialty, index) => (
                                            <ReadOnlyField
                                                key={index}
                                                fieldName={`doctorSpecialty${index + 1}`}
                                                fieldLabel={`Specialty ${index + 1}`}
                                                value={specialty || "N/A"}
                                                className="mt-2"
                                            />
                                        ))}
                                    {vm.isEditing && specialties.length < 2 && (
                                        <Button
                                            type="button"
                                            variant="ghost"
