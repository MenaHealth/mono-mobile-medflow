...
export function useForgotPasswordViewModel() {
    ...
    const sendResetEmail = useMutation(
        async () => {
            if (!selectedUser) throw new Error('No user selected');
            const response = await fetch('/api/admin/POST/send-password-reset-email', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${token}`,
    ...
    return {
        ...
        sendResetEmail: sendResetEmail.mutate,
        loading: loading || generateResetLink.isLoading || clearExpiredLinks.isLoading || sendResetEmail.isLoading,
        handleLogout,
    };
}
