...
export interface INote extends Document {
    email: string;
    noteType: 'physician' | 'procedure' | 'subjective' | 'blank';
    date: Date;
    authorName?: string;
    authorID?: string;
...
export const noteSchema = new Schema<INote>({
    ...
    noteType: { type: String, enum: ['physician', 'procedure', 'subjective', 'blank'], required: true },
...
export interface IBlankNote extends INote {
    content: {
        noteField: string;
    }
}
...
const BlankNote = Note.discriminators?.BlankNote || Note.discriminator<IBlankNote>('BlankNote', new Schema<IBlankNote>({
    content: {
        noteField: { type: String, required: true },
    }
}));

export { Note, PhysicianNote, ProcedureNote, SubjectiveNote, BlankNote };