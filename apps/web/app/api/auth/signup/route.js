...
export async function POST(request) {
    ...
    // Ensure doctorSpecialty is always an array with a single string
    const formattedDoctorSpecialty = Array.isArray(doctorSpecialty) && doctorSpecialty.length > 0
        ? [doctorSpecialty[0]]
        : [];
    ...
    try {
        ...
        const newUserData = {
            ...
            lastName,
            dob: new Date(dob),
            doctorSpecialty: formattedDoctorSpecialty, // Ensure specialty is always an array with a single string
            languages,
            countries,
            gender,
        };
